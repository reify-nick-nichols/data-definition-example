...
(deftest sheep->kg-test
  ...
  (testing "Generative tests"
    (let [generated-sheep (first (gen/sample (s/gen ::sut/sheep)))
...
(check.test/defspec sheep->kg-is-positive 10000
  (prop/for-all [test-sheep (s/gen ::sut/sheep)]
    (pos? (sheep/sheep->kg test-sheep))))

;; If the weight was in kilos, converting it wouldn't change the value
(check.test/defspec sheep->kg-identity-for-kilos 10000
  (prop/for-all [test-sheep (s/gen ::sut/sheep)]
...
(check.test/defspec sheep->kg-larger-for-pounds 10000
  (prop/for-all [test-sheep (s/gen ::sut/sheep)]
